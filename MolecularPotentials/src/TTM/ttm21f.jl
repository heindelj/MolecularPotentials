...
TTM21F(num_waters::Int) = TTM21F(num_waters, zeros(3, 4 * num_waters), TTM21_Constants(), Electrostatics(zeros(Int(num_waters * 4)), TTM21_Constants()))
...
function compute_M_site_crd!(ttm21f::TTM21F, coords::AbstractMatrix{Float64})
    ...
    for i in 1:Int(size(coords, 2) / 3) * 4
        if i % 4 == 0
            @views ttm21f.M_site_coords[:, i] = ttm21f.constants.γ_1 * coords[:,i-3 - num_M_sites] + ttm21f.constants.γ_2 * (coords[:,i-2 - num_M_sites] + coords[:, i-1 - num_M_sites])
            num_M_sites += 1
        else
            @views ttm21f.M_site_coords[:, i] = coords[:, (i - num_M_sites)]
        end
    end
end
